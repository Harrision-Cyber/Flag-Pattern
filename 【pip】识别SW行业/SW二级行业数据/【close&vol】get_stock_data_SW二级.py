...
df = pd.read_excel("C:/Users/Amber/Desktop/2025年策略Task/PA量化/旗形形态/【pip】识别全部A股/获取数据/SW二级行业/swlist.xlsx", sheet_name="list")
# 使用 tolist() 将结果转换为 Python 列表
stock_codes = df['Wind代码'].dropna().tolist()
stock_names = df['证券名称'].dropna().tolist()

# 获取指定股票列表的数据
all_data = get_stock_database(stock_codes, stock_names, "2010-01-01", "2025-03-31")
...
excel_path = os.path.join(excel_dir, "【close&vol】SW二级数据.xlsx")
