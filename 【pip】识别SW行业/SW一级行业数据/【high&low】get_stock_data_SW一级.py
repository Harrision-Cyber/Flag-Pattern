...
df = pd.read_excel("C:/Users/Amber/Desktop/2025年策略Task/PA量化/旗形形态/【pip】识别SW行业/SW一级行业数据/sw一级list.xlsx", sheet_name="list")
# 使用 tolist() 将结果转换为 Python 列表
stock_codes = df['Wind代码'].dropna().tolist()
stock_names = df['证券名称'].dropna().tolist()

# 获取指定股票列表的数据
all_data = get_stock_database(stock_codes, stock_names, "2000-01-01", "2025-03-31")

# 创建保存Excel文件的目录
excel_dir = '【high&low】SW一级数据'
if not os.path.exists(excel_dir):
    os.makedirs(excel_dir)

# 将所有股票数据保存到一个Excel文件，不同指标保存在不同sheet
excel_path = os.path.join(excel_dir, "【high&low】SW一级数据.xlsx")
with pd.ExcelWriter(excel_path, engine='openpyxl') as writer:
    all_data['high_df'].to_excel(writer, sheet_name='最高价')
    all_data['low_df'].to_excel(writer, sheet_name='最低价')
    # all_data['close_df'].to_excel(writer, sheet_name='收盘价')
    # all_data['volume_df'].to_excel(writer, sheet_name='成交量')

print(f"SW一级数据已保存至 {excel_path}")
